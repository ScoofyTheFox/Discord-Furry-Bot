...
const giveaway = {
  ...
  async execute(interaction, client) {
    ...
    if (!channel) {
      return interaction.reply({ embeds: [emb.error(client, 'No Channel', 'Set a giveaway channel with `/setup-channels`.')], flags: 64 });
    }
    ...
    await interaction.reply({ embeds: [emb.success(client, 'Giveaway Started! 🎉',
      channelId !== interaction.channelId
        ? `Posted in <#${channelId}>!`
        : `Ends <t:${Math.floor(endTime / 1000)}:R>`)], flags: 64 });
  },
};
...
const giveawayReroll = {
  ...
  async execute(interaction, client) {
    try {
      ...
      const reaction = msg?.reactions.cache.get('🎉');
      if (!reaction) return interaction.reply({ embeds: [emb.error(client, 'Not Found', 'Could not find that giveaway.')], flags: 64 });
      const users   = await reaction.users.fetch();
      const entries = users.filter(u => !u.bot).map(u => u);
      if (!entries.length) return interaction.reply({ embeds: [emb.warn(client, 'No Entries', 'No one to reroll.')], flags: 64 });
      const winner = entries[Math.floor(Math.random() * entries.length)];
      await interaction.reply({ embeds: [emb.success(client, '🎲 Rerolled!', `New winner: <@${winner.id}>!`)] });
    } catch {
      await interaction.reply({ embeds: [emb.error(client, 'Error', 'Could not reroll that giveaway.')], flags: 64 });
    }
  },
};
