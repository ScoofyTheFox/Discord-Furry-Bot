...
const help = {
  ...
  async execute(interaction, client) {
    ...
    while (i < sorted.length) {
      if (fields.length >= 24 && i < sorted.length - 1) {
        const remaining = sorted.slice(i)
          .map(([c, cs]) => `**${c}**\n${cs.join(' ')}`)
          .join('\n\n');
        const val = remaining.slice(0, 1024) || '…';
        fields.push({ name: 'More', value: val });
        break;
      }
      const [cat, cmds] = sorted[i];
    ...
    // Split into chunks of 25 fields max (Discord limit)
    const chunks = [];
    for (let j = 0; j < fields.length; j += 25) chunks.push(fields.slice(j, j + 25));

    const embed = emb.base(client)
      .setTitle(`📖 ${client.settings.bot_name} — Commands`)
      .setDescription(`**${client.commands.size}** commands available`);

    // Add first chunk (only embed we send)
