...
const kick = {
  ...
  async execute(interaction, client) {
    ...
    if (!target?.kickable) return interaction.reply({ embeds: [emb.error(client, 'Cannot Kick', 'I cannot kick that user.')], ephemeral: true });
...
const ban = {
  ...
  async execute(interaction, client) {
    ...
    if (!target?.bannable) return interaction.reply({ embeds: [emb.error(client, 'Cannot Ban', 'I cannot ban that user.')], ephemeral: true });
...
const unban = {
  ...
  async execute(interaction, client) {
    ...
    try {
      ...
      await interaction.reply({ embeds: [emb.error(client, 'Failed', 'User not found in ban list.')], ephemeral: true });
    }
  },
};
...
const warnings = {
  ...
  async execute(interaction, client) {
    const target = interaction.options.getUser('user');
    const warns  = client.warnings.get(target.id) || [];
    if (!warns.length) return interaction.reply({ embeds: [emb.success(client, 'Clean', `**${target.tag}** has no warnings this session.`)], ephemeral: true });
    const lines = warns.map((w, i) => `**${i + 1}.** ${w.reason} — *${w.mod}*`);
    await interaction.reply({ embeds: [emb.warn(client, `Warnings — ${target.tag}`, lines.join('\n') + '\n\n*Resets on bot restart.*')], ephemeral: true });
  },
};
...
const clearwarns = {
  ...
  async execute(interaction, client) {
    ...
    await interaction.reply({ embeds: [emb.success(client, 'Cleared', `Warnings cleared for **${target.tag}**`)], ephemeral: true });
  },
};
