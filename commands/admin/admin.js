...
const adminSay = {
  ...
  async execute(interaction, client) {
    ...
    await ch.send(msg);
    await interaction.reply({ embeds: [emb.success(client, 'Sent', `Message sent to <#${ch.id}>`)], ephemeral: true });
  },
};
...
const adminEmbed = {
  ...
  async execute(interaction, client) {
    ...
    await interaction.reply({ embeds: [emb.success(client, 'Sent', `Embed sent to <#${ch.id}>`)], ephemeral: true });
  },
};
...
const adminDm = {
  ...
  async execute(interaction, client) {
    ...
    try {
      ...
      await interaction.reply({ embeds: [emb.success(client, 'DM Sent', `Sent to **${target.tag}**`)], ephemeral: true });
    } catch {
      await interaction.reply({ embeds: [emb.error(client, 'Failed', `**${target.tag}** has DMs disabled.`)], ephemeral: true });
    }
  },
};
...
const adminAnnounce = {
  ...
  async execute(interaction, client) {
    ...
    if (!channelId) return interaction.reply({ embeds: [emb.error(client, 'No Channel', 'Set one with `/setup-channels`.')], ephemeral: true });

    const ch = interaction.guild.channels.cache.get(channelId);
    if (!ch) return interaction.reply({ embeds: [emb.error(client, 'Not Found', 'Channel not found.')], ephemeral: true });
    ...
    await interaction.reply({ embeds: [emb.success(client, 'Announced', `Posted to <#${channelId}>`)], ephemeral: true });
  },
};
...
const adminGiveRole = {
  ...
  async execute(interaction, client) {
    ...
    if (everyone) {
      await interaction.reply({ embeds: [emb.info(client, 'Processing...', `Giving **${role.name}** to all members.`)], ephemeral: true });
      ...
      await interaction.reply({ embeds: [emb.success(client, 'Role Given', `<@&${role.id}> → **${target.user.tag}**`)], ephemeral: true });
    } else {
      await interaction.reply({ embeds: [emb.error(client, 'No Target', 'Specify a user or set `everyone: true`')], ephemeral: true });
    }
  },
};
...
const adminRemoveRole = {
  ...
  async execute(interaction, client) {
    ...
    if (everyone) {
      await interaction.reply({ embeds: [emb.info(client, 'Processing...', `Removing **${role.name}** from all.`)], ephemeral: true });
      const members = await interaction.guild.members.fetch();
      let count = 0;
      ...
      await interaction.reply({ embeds: [emb.success(client, 'Role Removed', `Removed from **${target.user.tag}**`)], ephemeral: true });
    } else {
      await interaction.reply({ embeds: [emb.error(client, 'No Target', 'Specify a user or set `everyone: true`')], ephemeral: true });
    }
  },
...
const adminNick = {
  ...
  async execute(interaction, client) {
    ...
    await interaction.reply({ embeds: [emb.success(client, 'Updated', nick ? `Nickname set to **${nick}**` : 'Nickname reset')], ephemeral: true });
  },
};
...
const adminSetstatus = {
  ...
  async execute(interaction, client) {
    ...
    await interaction.reply({ embeds: [emb.success(client, 'Status Updated 💾', `Now **${type}** "${status}"`)], ephemeral: true });
  },
};
