...
const fursuitCheck = {
  ...
  async execute(interaction, client) {
    ...
    if (!channelId) {
      return interaction.reply({ embeds: [emb.error(client, 'Not Configured', 'Set a fursuit channel with `/setup-channels`.')], ephemeral: true });
    }

    const channel = interaction.guild.channels.cache.get(channelId);
    if (!channel) {
      return interaction.reply({ embeds: [emb.error(client, 'Not Found', 'Fursuit channel not found. Check `/setup-channels`.')], ephemeral: true });
    }
    ...
    await interaction.reply({ embeds: [emb.success(client, 'Submitted! 🐾', `Posted to <#${channelId}>!`)], ephemeral: true });
  },
};
