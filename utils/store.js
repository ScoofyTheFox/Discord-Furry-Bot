...
async function load() {
  if (!KEY) {
    ...
    const hasManualRoles    = Object.values(defaults.roles    || {}).some(v => v && v.trim() !== '');
    const hasManual = hasManualChannels || hasManualRoles;

    console.warn('\n┌──────────────────────────────────────────────────────┐');
    console.warn(  '│  ⚠️  JSONBIN_KEY not set                              │');
    console.warn(  '│                                                       │');
    console.warn(  '│  You can still use /setup-channels & /setup-roles     │');
    console.warn(  '│  but settings WON\'T survive a restart.               │');
    console.warn(  '│                                                       │');
    console.warn(  '│  To persist settings, pick one:                       │');
    console.warn(  '│    1. Set IDs manually in settings.js                 │');
    console.warn(  '│    2. Add JSONBIN_KEY to .env  →  jsonbin.io (free)   │');
    console.warn(  '└──────────────────────────────────────────────────────┘');
    if (hasManual) {
      console.warn('✅ Manual IDs found in settings.js — those will work fine.\n');
    } else {
      console.warn('❌ No manual IDs set in settings.js — channels/roles will be empty until configured.\n');
    }
    return;
  }
